#歌曲id，在列表页里有
'''
搜索位置：network --> Media；network --> all -->fcg_music
'''
#1.查找多首歌之间的连接有什么不同？(查找位置 network --> Media)
...
#不同点，歌曲名字不同，vkey不同，
#（1）对比列表页发现，列表页的歌曲连接里有 https://y.qq.com/n/yqq/song/001BqoH92Me5XP.html
#001BqoH92Me5XP  跟 真正的下载歌曲连接 filename部分 相同，只是拼接了  C400++.m4a
#（2）vkey 在哪？
#在控制台network --> all -->fcg_music里找到了一个url连接返回值带有 filename和vkey的
#（3）这下，看一下我们的请求链接。注意搜索中可以使用“find”的。
...
# guid  请求时的guid要和歌曲的下载链接处的guid相同。猜测为匿名用户的一个匿名ID。此处为674695650。

# songmid: 0032ZOkm0LBgHW----------songmid: 004JCcIC4OCi1Q
# filename: C4000032ZOkm0LBgHW.m4a----------filename: C400004JCcIC4OCi1Q.m4a
#分析：9211762249380087/31831938060021003/4381652361789885 在文件中找不到，怎么办？我们看一下它是否有用？于是用一个随机数去试试。
...
def song_id_get_download_url(songname,songid="004JCcIC4OCi1Q"):
    ...
    callback="MusicJsonCallback"+str(random.random())[2:10] #random.random()产生一个0-1之间的随机数
    ...
    vkey_response=requests.get(url=getvkeyurl,params=getvkeyparams,headers=headers)#post时，params参数好像应该换成key
...
#获取一个歌单里所有的音乐名称和id,动态爬取：network-->all-->fcg.*
def songlist(disstid="3851947927"):
    ...
    for one_song in songs_list:
        ...
        songname=re.sub("[/\|\*\?\">:<\\\\]",'',songname) #把名字中的特殊字符换掉
...
if __name__=="__main__":
    #风格版专辑下载
    song_list=songlist("3851947927")
    for one_song in song_list['song_list']:
        songinfo = song_id_get_download_url(one_song["songname"],one_song["songmid"])
        download_music(songinfo,song_list["dissname"])


    #作者版专辑搜索下载
    for page in range(1,3):
        search_song=search_list("鹿先森乐队",page)
